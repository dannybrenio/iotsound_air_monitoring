<x-layout>
            ...
            <div class="flex flex-row bg-[#eceaea] rounded-t-lg overflow-x-auto">
              @foreach($hardwares as $hw)
                ...
                <a href="{{ route('dashboard', ['hardware' => $hw->public_hash]) }}"
                   @click.prevent="
                      if (activeTab === 'weather' && {{ $isSelected ? 'true' : 'false' }}) {
                          activeTab = 'barangay'
                      } else {
  ...
  <script>
      ...
      * ========================= */
      ...
      /* build labels directly from the segments */
      const labels12h = sorted12h.map(s => s.label || fmtTime(s.start));
      const labels24hRaw = sorted24h.map(s => s.label || fmtTime(s.start));
      ...
      * ========================= */
      ...
      /* =========================
      * SAFE LOCAL CACHE (adds, does not replace UI logic)
      * ========================= */
      const DASH_CACHE_TTL = 5 * 60 * 1000; // 5 min
      const dashboardCacheKey = `dashboard_data_${selectedHardwareId}`;
      ...
      function dashCacheGet() {
        try {
          ...
          if (!obj?.cachedAt) return null;
          if (Date.now() - obj.cachedAt > DASH_CACHE_TTL) {
      ...
      function dashCacheSet(payload) {
        try {
          localStorage.setItem(dashboardCacheKey, JSON.stringify({ ...payload, cachedAt: Date.now() }));
        } catch (_) {}
      }
    
      // Apply cached values ONLY to the basic “card” vars (no chart/slider rewiring)
      (function primeFromCacheIfAny() {
        const cached = dashCacheGet();
        if (!cached) return;
    
        // Only patch values your initial render uses for text fields.
        if (cached.latestVals) {
          latestVals.pm2_5 = cached.latestVals.pm2_5 ?? latestVals.pm2_5;
          latestVals.pm10  = cached.latestVals.pm10  ?? latestVals.pm10;
          latestVals.co    = cached.latestVals.co    ?? latestVals.co;
          latestVals.no2   = cached.latestVals.no2   ?? latestVals.no2;
        }
        // (Optional) you can also patch decibel numbers if you want
        // but we’ll keep it conservative to avoid UI side effects.
      })();
      ...
      document.addEventListener('DOMContentLoaded', () => {
        ...
        // ✅ save initial card values to cache (safe; does not touch charts/slider)
        dashCacheSet({
          latestVals,
          latestDecibel,
          peakDecibel,
      ...
      * ========================= */
      ...
      function appendPoint(rangeKey, iso, nowcast, decibel) {
        ...
        if (decibel != null) {
          ...
          buf.peakDb[i] = toNum(decibel) != null ? toNum(decibel) + 5 : null;
        }
      }
        ...
        window.Echo.channel('dashboard')
          .listen('DashboardUpdated', (d) => {
            if (d.hardware_id == selectedHardwareId) {
              ...
              // ✅ update cache on realtime update (safe, no UI side effects)
              dashCacheSet({
                latestVals: d.latest_vals ?? null,
                latestDecibel: d.latest_decibel ?? null,
                peakDecibel: d.peak_decibel ?? null,
                averageDecibel: d.average_decibel ?? null,
                latestDateTime: d.latest_datetime ?? null
              });
            }
          });
      ...
      * ========================= */
      async function loadPrediction() {
          ...
          <div class="w-70 h-auto flex justify-center items-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 24 24">
              <circle cx="12" cy="3.5" r="1.5" fill="#000000" opacity="0">
                <animateTransform attributeName="transform" calcMode="discrete" dur="2.4s" repeatCount="indefinite" type="rotate"
              ...
              <circle cx="12" cy="3.5" r="1.5" fill="#000000" opacity="0">
                <animateTransform attributeName="transform" begin="0.2s" calcMode="discrete" dur="2.4s" repeatCount="indefinite" type="rotate"
              ...
              <circle cx="12" cy="3.5" r="1.5" fill="#000000" opacity="0">
                <animateTransform attributeName="transform" begin="0.4s" calcMode="discrete" dur="2.4s" repeatCount="indefinite" type="rotate"
        ...
        `;
        noiseEl.innerHTML = aqiEl.innerHTML;
