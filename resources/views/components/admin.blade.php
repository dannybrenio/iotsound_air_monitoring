...
<body>
    ...
    <!-- Sidebar -->
    <div id="mobile-menu" class="bg-[#E0EBDC] fixed h-full w-[65%] lg:w-[20%] flex flex-col justify-between items-center py-6 gap-y-5 
        shadow-[0px_0_15px_1px_rgba(0,0,0,0.3)] z-40">
        <!-- Add Close Button Here -->
        <button id="close-menu"
            class="absolute top-5 right-4 text-gray-700 text-4xl lg:hidden cursor-pointer">&times;</button>
        <div class="flex flex-row justify-start items-center gap-x-2 w-[85%] flex-shrink-0">
            <img src="{{ asset('bglogo.png') }}" alt="" class="size-8">
            <span class="text-[#06402b] font-extrabold text-lg">AEROSON</span>
        </div>
        <!-- <div class="rounded border border-[#06402b] w-[85%]"></div> -->
        <div
            class="flex flex-col w-[85%] justify-start h-full items-start gap-y-1 flex-1 min-h-0 overflow-y-auto sidebar-scroll">
            <div class="flex items-center w-full justify-center flex-shrink-0">
                <span class="text-sm text-[#06402b] uppercase mb-2 font-bold">Admin Panel</span>
            </div>
            <div class="flex flex-col gap-y-1 w-full">
                <a href="{{ route('adminDashboard') }}"
                    class="flex flex-row gap-x-3 justify-center items-center text-xs truncate h-8
                            {{ request()->routeIs('adminDashboard') ? 'text-white  duration-300 bg-[#06402b] font-semibold w-full rounded justify-start px-2' : 'text-[#06402b] font-semibold hover:text-black hover:bg-[#a9b3da] w-full rounded flex justify-start duration-300 px-2' }}">
                ...
                <a href="{{ route('hardware') }}"
                    ...
                    <svg fill="currentColor" class="size-4" viewBox="0 0 15 15" version="1.1" id="hardware"
                ...
                <a href="{{ route('pendingHardware') }}"
                    ...
                    <svg fill="currentColor" class="size-4" viewBox="0 0 32 32" id="icon"
                        xmlns="http://www.w3.org/2000/svg">
                        <defs>
                            <style>
                                .cls-1 {
                ...
                <a href="{{ route('pendingData') }}"
                    ...
                    <svg class="size-4" viewBox="0 0 1024 1024" class="icon" version="1.1"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                ...
                <a href="{{ route('report') }}"
                    ...
                    <svg fill="currentColor" class="size-4" viewBox="0 0 32 32"
                ...
                <a href="{{ route('account') }}"
                    ...
                    <svg class="size-4" viewBox="0 0 24 24" id="Layer_1" data-name="Layer 1"
                        xmlns="http://www.w3.org/2000/svg">
                        <defs>
                            <style>
                                .cls-1 {
                ...
                <a href="{{ route('alert') }}"
                    ...
                    <svg class="size-4" viewBox="-1 0 30 30" version="1.1" xmlns="http://www.w3.org/2000/svg"
        ...
        <div
            class="w-[80%] h-auto flex flex-col justify-between items-center gap-y-2 flex-shrink-0 rounded-xl">
            <!--<div class="flex flex-col items-start w-[80%] gap-y-0">-->
            <!--    <span class="text-base text-[#06402b] uppercase font-black">Admin</span>-->
            <!--    <span class="text-xs text-[#06402b]">Admin Manager</span>-->
            <!--</div>-->
            <!-- <div class="rounded border border-[#FFFFFF] w-[80%]"></div> -->
            <a href="{{ route('logout') }}"
                class="px-2.5 text-[#FFFFFF] text-xs flex flex-row gap-x-1 bg-[#06402b] items-center font-semibold hover:text-[#06402b] hover:bg-[#a9b3da] w-full h-10 rounded justify-center duration-300">
    ...
    <main class="lg:pl-[20%] pl-0">
        ...
        <div x-data="notifBell" class="fixed right-5 top-5">
            <!-- Notification Button -->
            <button @click="toggleNotif()"
                class="flex items-center justify-center h-10 w-10 bg-[#06402b] rounded-full text-white hover:bg-[#06402b]/80 duration-300 relative focus:bg-white focus:text-[#06402b] focus:ring-2 focus:ring-[#06402b] cursor-pointer">
                ...
                <span id="notifBadge" x-show="badge > 0" x-text="badge"
                    class="absolute -top-2 -right-2 bg-red-600 text-white text-xs font-semibold rounded-full px-1.5 py-0.5"></span>
            </button>

            <!-- Notification Modal -->
            <div x-show="open" @click.outside="closeNotif()" x-transition
                ...
                <div class="flex items-center justify-between p-3 border-b bg-[#06402b] text-white rounded-t-lg">
                    ...
                    <div class="relative group inline-block">
                        <a href="{{ route('history') }}">
                            <svg class="size-4" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" fill="none">
                        ...
                        <span
                            class="absolute bottom-full -right-12 -translate-x-1/2 mb-1 hidden group-hover:block bg-gray-700 text-white text-xs rounded py-1 px-2 whitespace-nowrap shadow-lg">
                            Full Notification
                        </span>
                    </div>
                    <!-- <button @click="open = false" class="text-sm">✕</button> -->
                </div>
                <div id="notifList" class="max-h-120 overflow-y-auto p-3 space-y-2 sidebar-scroll z-10">
                    @forelse($notifs as $notif)
                        <div class="p-2 rounded-md border border-gray-200 hover:bg-gray-100 cursor-pointer"
                            ...
                            <p class="text-sm text-gray-800">
                                ...
                                <span>{{ $notif->sensor_type }} {{ $notif->sensor_status }}</span>
                            </p>
                            <p class="text-xs text-gray-500">{{ $notif->created_at->diffForHumans() }}</p>
                        </div>
                    @empty
                        <div class="py-4 text-sm text-gray-500" data-empty>No unread notifications.</div>
                    @endforelse
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function getBadgeCount() {
            ...
            const n = parseInt(notifBadge.textContent, 10);
        ...
        Alpine.data('notifBell', () => ({
            ...
            markAllRead() {
                fetch('/notifications/mark-all-read', {
                    ...
                    .then(() => {
                        this.badge = 0; // update UI immediately
                    })
